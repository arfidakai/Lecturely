...
import { useLanguage } from "../contexts/LanguageContext"; // sesuaikan path
    ...
    <div className="min-h-screen bg-gradient-to-b from-purple-100 to-white py-8 px-4">
      <div className="max-w-md mx-auto w-full">
        ...
        <div className="flex items-center gap-4 mb-6 relative">
          ...
          <h1 className="text-2xl font-bold text-gray-900 flex-1">{t.subjects.title}</h1>
          <button
            ...
            aria-label={t.subjects.addSubject}
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400 bg-white rounded-2xl shadow-sm">
            {searchQuery ? t.subjects.noSubjectsDesc : t.subjects.noSubjects}
          </div>
        ) : (
              ...
              <div className="absolute inset-0 bg-black/30 flex items-center justify-center px-6 backdrop-blur-sm z-30">
                <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg text-gray-900">{t.common.edit} {t.subjects.title}</h3>
                  ...
                  <input
                    ...
                    placeholder={t.subjects.lecturer}
                    className="w-full px-4 py-3 bg-gray-50 rounded-xl border border-gray-200 focus:border-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-100 transition-all mb-3"
                  />
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {colorOptions.map((color) => (
                        <button key={color} type="button"
                          ...
                          onClick={() => setEditColor(color)} aria-label={color} />
                      ))}
                    </div>
                  </div>
                  <div className="mb-4">
                    <div className="mb-1 text-sm text-gray-700">{t.subjects.scheduleDays}</div>
                    <div className="flex flex-wrap gap-2">
                      {dayOptions.map((day) => (
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setShowEditModal(false)}
                      ...
                      disabled={editing}>{t.common.cancel}</button>
        ...
        {showAddModal && (
          <div className="absolute inset-0 bg-black/30 flex items-center justify-center px-6 backdrop-blur-sm z-30">
            <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg text-gray-900">{t.subjects.addSubject}</h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {colorOptions.map((color) => (
                    <button key={color} type="button"
                      ...
                      style={{ backgroundColor: color }}
                      onClick={() => setNewColor(color)} aria-label={color} />
                  ))}
                </div>
              </div>
              <div className="mb-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowAddModal(false)}
                  className="flex-1 px-4 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-colors"
                  disabled={adding}>{t.common.cancel}</button>
                <button onClick={handleAddSubject}
                  disabled={!newSubjectName.trim() || !newLecturer.trim() || !newIcon || !newColor || adding}
                  className="flex-1 px-4 py-3 bg-purple-500 text-white rounded-xl hover:bg-purple-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  {adding ? t.common.loading : t.subjects.addSubject}
                </button>
              </div>
            </div>
          </div>
